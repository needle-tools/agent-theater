...
- **Optimization & compression run at build time (not a scene upload).** Texture (KTX2/WebP) and mesh (Draco/Meshopt) compression, progressive loading, and automatic LODs are applied during the production build (Compression & LOD Settings component). It's **license-gated**: sign in to Needle Cloud (`npx needle-cloud login`; the Unity/Blender integrations handle this) or, for CI, set `NEEDLE_CLOUD_TOKEN` to an `nc_*` access token. It's a license check, not a scene upload. Details: [Optimization & Compression](./references/optimization.md).
